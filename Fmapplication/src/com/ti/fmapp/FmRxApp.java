...
public class FmRxApp extends Activity implements View.OnClickListener,
    ...
    private void startup() {
        ...
        printDebugInfo = Preferences.getPrintDebugInfo(FmRxApp.this);
        ...
        switch (sFmReceiver.getFMState()) {
            ...
            case FmReceiver.STATE_DISABLED:
                Utils.debugFunc("FmReceiver.STATE_DISABLED",Log.INFO,printDebugInfo);
            ...
            case FmReceiver.STATE_DEFAULT:
                ...
                */
                if (!mFmInterrupted) {
                    ...
                    Utils.debugFunc("mFmInterrupted is true dont call enable",Log.INFO,printDebugInfo);
                }
                break;
        }
    }
    ...
    public void onServiceConnected() {
        Utils.debugFunc("onServiceConnected",Log.INFO,printDebugInfo);
        mFmServiceConnected = true;
        startup();
    }

    public void onServiceDisconnected() {
        Utils.debugFunc("Lost connection to service",Log.INFO,printDebugInfo);
            ...
            switch (msg.what) {
                ...
                */
                case EVENT_FM_ENABLED:
                    Utils.debugFunc("enter handleMessage ----EVENT_FM_ENABLED",Log.INFO,printDebugInfo);
                    if (pd != null) {
                        pd.dismiss();
                    }
                ...
                */
                case EVENT_MONO_STEREO_CHANGE:
                    Utils.debugFunc( "enter handleMessage ---EVENT_MONO_STEREO_CHANGE",Log.INFO,printDebugInfo);
                    if (mMode == 0) {
                ...
                */
                case EVENT_MONO_STEREO_DISPLAY:
                    Utils.debugFunc( "enter handleMessage ---EVENT_MONO_STEREO_DISPLAY",Log.INFO,printDebugInfo);
                ...
                */
                case EVENT_MUTE_CHANGE:
                    Utils.debugFunc("enter handleMessage ---EVENT_MUTE_CHANGE",Log.INFO,printDebugInfo);
                    break;

                case EVENT_SEEK_STOPPED:
                    Integer seekFreq = (Integer) msg.obj;
                    Utils.debugFunc("enter handleMessage ----EVENT_SEEK_STOPPED seekFreq: " + seekFreq,Log.INFO,printDebugInfo);
                ...
                case EVENT_FM_DISABLED:
                    Utils.debugFunc("enter handleMessage ----EVENT_FM_DISABLED",Log.INFO,printDebugInfo);
                    /*
                ...
                case EVENT_SEEK_STARTED:
                    ...
                    Utils.debugFunc("enter handleMessage ----EVENT_SEEK_STARTED freq" + freq,Log.INFO,printDebugInfo);
                ...
                */
                case EVENT_VOLUME_CHANGE:
                    Utils.debugFunc("enter handleMessage ----EVENT_VOLUME_CHANGE",Log.INFO,printDebugInfo);
                    /*
                ...
                case EVENT_COMPLETE_SCAN_PROGRESS:
                    Utils.debugFunc("enter handleMessage ----EVENT_COMPLETE_SCAN_PROGRESS",Log.INFO,printDebugInfo);

                    Integer progress = (Integer) msg.obj;
                    Utils.debugFunc("enter handleMessage ----EVENT_COMPLETE_SCAN_PROGRESS progress" + progress,Log.INFO,printDebugInfo);
                    break;
                ...
                */
                case EVENT_TUNE_COMPLETE:
                    ...
                    Utils.debugFunc("enter handleMessage ----EVENT_TUNE_COMPLETE tuneFreq"+ tuneFreq,Log.INFO,printDebugInfo);
                    ...
                    // clear the PS text
                    txtPsText.setText(null);
                    Utils.debugFunc("sdefaultSettingOn: " + sdefaultSettingOn,Log.INFO,printDebugInfo);
...
*/
                ...
                case EVENT_RDS_TEXT:
                    ...
                    if (FM_SEND_RDS_IN_BYTEARRAY) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            Utils.debugFunc("rdsText" + rdsText[i],Log.INFO,printDebugInfo);
                        }
                    } else {
                ...
                case EVENT_PI_CODE:
                    ...
                    Utils.debugFunc("enter handleMessage ----EVENT_PI_CODE rds" + pi,Log.INFO,printDebugInfo);
                    break;

                case EVENT_SET_CHANNELSPACE:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_CHANNELSPACE",Log.INFO,printDebugInfo);
                    break;


                case EVENT_GET_CHANNEL_SPACE_CHANGE:
                    Utils.debugFunc("enter handleMessage ----EVENT_GET_CHANNEL_SPACE_CHANGE",Log.INFO,printDebugInfo);
                    Long gChSpace = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gChSpace" + gChSpace,Log.INFO,printDebugInfo);
                    break;

                /* tune to default frequency after the band change callback . */

                case EVENT_BAND_CHANGE:
                    Utils.debugFunc("enter handleMessage ----EVENT_BAND_CHANGE",Log.INFO,printDebugInfo);
                    /*
                ...
                case EVENT_ENABLE_RDS:
                    Utils.debugFunc( "enter handleMessage ----EVENT_ENABLE_RDS",Log.INFO,printDebugInfo);
                    break;

                /* Set RSSI after SET_RDS_AF callback */
                case EVENT_SET_RDS_AF:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_RDS_AF",Log.INFO,printDebugInfo);
                    break;
                /* Set RDS AF after SET_RDS_SYSTEM callback */
                case EVENT_SET_RDS_SYSTEM:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_RDS_SYSTEM",Log.INFO,printDebugInfo);
                    break;
                /* Set RSSI after disable RDS callback */
                case EVENT_DISABLE_RDS:
                    Utils.debugFunc( "enter handleMessage ----EVENT_DISABLE_RDS",Log.INFO,printDebugInfo);
                    txtPsText.setText(null);
                    txtRadioText.setText(null);
                    break;

                case EVENT_SET_DEEMP_FILTER:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_DEEMP_FILTER",Log.INFO,printDebugInfo);
                    break;
                ...
                case EVENT_SET_RSSI_THRESHHOLD:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_RSSI_THRESHHOLD",Log.INFO,printDebugInfo);
                    /*
                ...
                case EVENT_SET_RF_DEPENDENT_MUTE:
                    Utils.debugFunc("enter handleMessage ----EVENT_SET_RF_DEPENDENT_MUTE",Log.INFO,printDebugInfo);
                    break;

                case EVENT_COMPLETE_SCAN_STOP:
                    Utils.debugFunc("enter handleMessage ----EVENT_COMPLETE_SCAN_STOP",Log.INFO,printDebugInfo);
                    break;

                case EVENT_COMPLETE_SCAN_DONE:
                    Utils.debugFunc("enter handleMessage ----EVENT_COMPLETE_SCAN_DONE",Log.INFO,printDebugInfo);

                    int[] channelList = (int[]) msg.obj;
                    int noOfChannels = msg.arg2;
                    Utils.debugFunc("noOfChannels" + noOfChannels,Log.DEBUG,printDebugInfo);

                    for (int i = 0; i < noOfChannels; i++) {
                        Utils.debugFunc("channelList" + channelList[i],Log.DEBUG,printDebugInfo);
                    }
                ...
                case EVENT_GET_BAND:
                    ...
                    Utils.debugFunc("enter handleMessage ----gBand" + gBand,Log.DEBUG,printDebugInfo);
                    break;

                case EVENT_GET_FREQUENCY:

                    Integer gFreq = (Integer) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gFreq" + gFreq,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_VOLUME:
                    Long gVol = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gVol" + gVol,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_MODE:
                    Long gMode = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gMode" + gMode,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_MUTE_MODE:
                    ...
                    Utils.debugFunc("enter handleMessage ----gMuteMode" + gMuteMode,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_RF_MUTE_MODE:

                    Long gRfMuteMode = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gRfMuteMode" + gRfMuteMode,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_RSSI_THRESHHOLD:
                    Long gRssi = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gRssi" + gRssi,Log.DEBUG,printDebugInfo);
                    break;

                case EVENT_GET_RSSI:
                    Integer rssi = (Integer) msg.obj;
                    Utils.debugFunc("enter handleMessage ----rssi" + rssi,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_DEEMPHASIS_FILTER:
                    Long gFilter = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gFilter" + gFilter,Log.DEBUG,printDebugInfo);
                    break;

                case EVENT_GET_RDS_SYSTEM:
                    Long gRdsSys = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gRdsSys" + gRdsSys,Log.DEBUG,printDebugInfo);
                    break;
                case EVENT_GET_RDS_GROUPMASK:
                    Long gRdsMask = (Long) msg.obj;
                    Utils.debugFunc("enter handleMessage ----gRdsMask" + gRdsMask,Log.INFO,printDebugInfo);
                    break;

                case EVENT_MASTER_VOLUME_CHANGED:
                    Integer vol = (Integer) msg.obj;
                    mVolume = vol;
                    Utils.debugFunc("enter handleMessage ----mVolume" + vol,Log.INFO,printDebugInfo);

                    break;

                case EVENT_FM_ERROR:
                    Utils.debugFunc( "enter handleMessage ----EVENT_FM_ERROR",Log.INFO,printDebugInfo);
    ...
    private void setRdsConfig() {
        Utils.debugFunc("setRdsConfig()-entered",Log.INFO,printDebugInfo);
        ...
        Utils.debugFunc("setRdsConfig()--- band= " + band,Log.INFO,printDebugInfo);
        ...
        // it again
        {
            ...
            if (MAKE_FM_APIS_BLOCKING) {
                ...
                if (!mStatus) {
                    Utils.debugFunc("setRdsConfig()-- setBand ->Error",Log.ERROR,printDebugInfo);
        ...
        // already do not set it again
        {
            ...
            Utils.debugFunc( "setRdsConfig()--- DeEmp= " + deEmp,Log.INFO,printDebugInfo);

            if (!mStatus) {
                Utils.debugFunc("setRdsConfig()-- setDeEmphasisFilter ->Error",Log.ERROR,printDebugInfo);
                showAlert(this, "FmReceiver",
        ...
        Utils.debugFunc("setChannelSpacing()--- channelSpace= " + channelSpace,Log.INFO,printDebugInfo);
        ...
        // it again
        {
            ...
            if (!mStatus) {
                Utils.debugFunc("setChannelSpacing()-- setChannelSpacing ->Error",Log.ERROR,printDebugInfo);
                showAlert(this, "FmReceiver",
        ...
        Utils.debugFunc("setRDS()--- rdsEnable= " + rdsEnable,Log.INFO,printDebugInfo);
        if (mRds != rdsEnable) {
            ...
            if (rdsEnable) {
                ...
                if (!mStatus) {
                    Utils.debugFunc("setRDS()-- enableRds() ->Error",Log.ERROR,printDebugInfo);
                ...
                if (!mStatus) {
                    Utils.debugFunc("setRDS()-- disableRds() ->Error",Log.ERROR,printDebugInfo);
                    showAlert(this, "FmReceiver", getString(R.string.not_able_disable_rds));
                } else {
                    Utils.debugFunc( "setRDS()-- disableRds() ->success",Log.ERROR,printDebugInfo);
        ...
        int rdsSystem = fmConfigPreferences.getInt(RDSSYSTEM,
                DEFAULT_RDS_SYSTEM);
        if (DBG) {
            Utils.debugFunc( "setRdsSystem()--- rdsSystem= " + rdsSystem,Log.DEBUG,printDebugInfo);
        }
        if (mRdsSystem != rdsSystem) {
            ...
            if (!mStatus) {
                Utils.debugFunc("setRdsSystem()-- setRdsSystem ->Error",Log.ERROR,printDebugInfo);
                showAlert(this, "FmReceiver",
        ...
        if (DBG) {
            Utils.debugFunc("setRdsAf()--- rdsAfSwitch= " + rdsAf,Log.DEBUG,printDebugInfo);
        }
        if (mRdsAf != rdsAfSwitch) {
            ...
            if (!mStatus) {
                Utils.debugFunc("setRdsAf()-- setRdsAfSwitchMode(1) ->Error",Log.ERROR,printDebugInfo);
        ...
        Utils.debugFunc("setRssi()-ENTER --- rssiThreshHold= " + rssiThreshHold,Log.INFO,printDebugInfo);
        ...
        Utils.debugFunc("setRdsConfig()-exit",Log.INFO,printDebugInfo);

    }

    /* Load the Default values from the preference when the application starts */
    private void loadDefaultConfiguration() {
        Utils.debugFunc("loadDefaultConfiguration()-entered",Log.INFO,printDebugInfo);
        ...
        if (DBG) {
            Utils.debugFunc(" Load default band " + sBand + "default volume" + mVolume + "last fre"
                                + lastTunedFrequency + "mode" + mMode + "mToggleMute" + mToggleMute + "mRdsState" + mRdsState,Log.DEBUG,printDebugInfo);
        }
    ...
    private void saveDefaultConfiguration() {
        Utils.debugFunc("saveDefaultConfiguration()-Entered",Log.INFO,printDebugInfo);
        ...
        if (DBG) {
            Utils.debugFunc(" save default band " + sBand + "default volume" + mVolume
                                + "last fre" + lastTunedFrequency + "mToggleMute",Log.DEBUG,printDebugInfo);
        }
        editor.commit();
    }
    ...
    private void initControls() {
        Utils.debugFunc("enter initControls",Log.INFO,printDebugInfo);

        imgFmMode = (ImageView) findViewById(R.id.imgMode);
        if (mMode == 0) {
            Utils.debugFunc("> setting stereo icon: " + mMode,Log.INFO,printDebugInfo);
            imgFmMode.setImageResource(R.drawable.fm_stereo);
        } else {
            Utils.debugFunc("> setting mono icon: " + mMode,Log.INFO,printDebugInfo);
            imgFmMode.setImageResource(R.drawable.fm_mono);
        }
        ...
        if (mToggleMute) {
            imgFmVolume.setImageResource(R.drawable.fm_volume_mute);
            Utils.debugFunc("> initControls  mute: " + mToggleMute,Log.INFO,printDebugInfo);
        } else {
            imgFmVolume.setImageResource(R.drawable.fm_volume);
            Utils.debugFunc("> initControls  mute: " + mToggleMute,Log.INFO,printDebugInfo);
        }
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    Utils.debugFunc("InsertDelayThread()-- Exception!",Log.ERROR,printDebugInfo);
                }
                // Dismiss the Dialog
                configPd.dismiss();
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Utils.debugFunc("onActivityResult called",Log.INFO,printDebugInfo);
        ...
        switch (requestCode) {
            ...
            case (ACTIVITY_CONFIG): {
                if (resultCode == Activity.RESULT_OK) {
                    Utils.debugFunc("ActivityFmRdsConfig configurationState "+ configurationState,Log.INFO,printDebugInfo);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_B:
                Utils.debugFunc("Testing getTunedFrequency()  returned Tuned Freq = " + sFmReceiver.getTunedFrequency(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_C:
                Utils.debugFunc("Testing getRssiThreshold()    returned RSSI threshold = " + sFmReceiver.getRssiThreshold(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_D:
                Utils.debugFunc("Testing getBand() returned Band  = " + sFmReceiver.getBand(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_E:
                Utils.debugFunc("Testing getDeEmphasisFilter()    returned De-emp  = " + sFmReceiver.getDeEmphasisFilter(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_F:
                Utils.debugFunc("Testing getMonoStereoMode() returned MonoStereo = " + sFmReceiver.getMonoStereoMode(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_G:
                Utils.debugFunc("Testing getMuteMode()  returned MuteMode = " + sFmReceiver.getMuteMode(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_H:
                Utils.debugFunc("Testing getRdsAfSwitchMode()    returned RdsAfSwitchMode = " + sFmReceiver.getRdsAfSwitchMode(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_I:
                Utils.debugFunc("Testing getRdsGroupMask() returned RdsGrpMask = " + sFmReceiver.getRdsGroupMask(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_J:
                Utils.debugFunc("Testing getRdsSystem() returned Rds System = "+ sFmReceiver.getRdsSystem(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_K:
                Utils.debugFunc("Testing getRfDependentMuteMode()    returned RfDepndtMuteMode = "+ sFmReceiver.getRfDependentMuteMode(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_L:
                Utils.debugFunc("Testing getRssi()    returned value = " + sFmReceiver.getRssi(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_M:
                Utils.debugFunc("Testing isValidChannel()    returned isValidChannel = "+ sFmReceiver.isValidChannel(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_N:
                Utils.debugFunc("Testing getFwVersion()    returned getFwVersion = "+ sFmReceiver.getFwVersion(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_O:
                Utils.debugFunc("Testing getChannelSpacing()    returned getChannelSpacing = " + sFmReceiver.getChannelSpacing(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_P:
                Utils.debugFunc("Testing completeScan()",Log.INFO,printDebugInfo);
                sFmReceiver.completeScan();
                return true;

            case KeyEvent.KEYCODE_Q:
                Utils.debugFunc("Testing getCompleteScanProgress()    returned scan progress = " + sFmReceiver.getCompleteScanProgress(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_R:
                Utils.debugFunc("Testing stopCompleteScan()    returned status = " + sFmReceiver.stopCompleteScan(),Log.INFO,printDebugInfo);
                return true;

            case KeyEvent.KEYCODE_S:
                Utils.debugFunc( "Testing setRfDependentMuteMode()    returned RfDepndtMuteMode = "+
                        sFmReceiver.setRfDependentMuteMode((sFmReceiver.getRfDependentMuteMode() == 1) ? 0 : 1),Log.INFO,printDebugInfo);
                return true;
        }
        return false;
    }
    ...
    void tuneStationFrequency(String text, String name) {
        try {
            ...
            if (iFreq != 0) {
                ...
                Utils.debugFunc("lastTunedFrequency" + lastTunedFrequency,Log.INFO,printDebugInfo);
            ...
            Utils.debugFunc("nfe",Log.INFO,printDebugInfo);
        }
    }
    ...
    protected void onSaveInstanceState(Bundle icicle) {
        ...
        Utils.debugFunc("onSaveInstanceState",Log.INFO,printDebugInfo);
    ...
    public void onStart() {
        Utils.debugFunc("onStart",Log.INFO,printDebugInfo);
        super.onStart();
    }

    public void onPause() {
        super.onPause();
        Utils.debugFunc("onPause",Log.INFO,printDebugInfo);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        Utils.debugFunc("onConfigurationChanged",Log.INFO,printDebugInfo);
        super.onConfigurationChanged(newConfig);

    }

    public void onResume() {
        Utils.debugFunc("onResume",Log.INFO,printDebugInfo);
        super.onResume();
    ...
    public void onDestroy() {
        Utils.debugFunc("onDestroy",Log.INFO,printDebugInfo);
        super.onDestroy();
        /*
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Utils.debugFunc("enter onReceive" + fmAction,Log.INFO,printDebugInfo);
            if (fmAction.equals(FmReceiverIntent.FM_ENABLED_ACTION)) {
                Utils.debugFunc("enter onReceive FM_ENABLED_ACTION " + fmAction,Log.INFO,printDebugInfo);

                mHandler.sendMessage(mHandler
                        .obtainMessage(EVENT_FM_ENABLED, 0));
            }
            if (fmAction.equals(FmReceiverIntent.FM_DISABLED_ACTION)) {
                Utils.debugFunc("enter onReceive FM_DISABLED_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_MODE_MONO_STEREO_ACTION)) {
                Utils.debugFunc("enter onReceive SET_MODE_MONO_STEREO_ACTION "+ fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction
                ...
                Utils.debugFunc( "enter onReceive DISPLAY_MODE_MONO_STEREO_ACTION "+ fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.PI_CODE_CHANGED_ACTION)) {
                Utils.debugFunc("enter onReceive PI_CODE_CHANGED_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.TUNE_COMPLETE_ACTION)) {
                Utils.debugFunc("enter onReceive TUNE_COMPLETE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.COMPLETE_SCAN_PROGRESS_ACTION)) {
                Utils.debugFunc("enter onReceive COMPLETE_SCAN_PROGRESS_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.VOLUME_CHANGED_ACTION)) {
                Utils.debugFunc("enter onReceive VOLUME_CHANGED_ACTION " + fmAction,Log.INFO,printDebugInfo);
                mHandler.sendMessage(mHandler.obtainMessage(
                        EVENT_VOLUME_CHANGE, 0));
            }

            if (fmAction.equals(FmReceiverIntent.MUTE_CHANGE_ACTION)) {
                Utils.debugFunc("enter onReceive MUTE_CHANGE_ACTION " + fmAction,Log.INFO,printDebugInfo);
                mHandler.sendMessage(mHandler.obtainMessage(EVENT_MUTE_CHANGE,
                        0));
            }

            if (fmAction.equals(FmReceiverIntent.SEEK_STOP_ACTION)) {
                Utils.debugFunc( "enter onReceive SEEK_STOP_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SEEK_ACTION)) {
                Utils.debugFunc("enter onReceive SEEK_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.BAND_CHANGE_ACTION)) {
                Utils.debugFunc("enter onReceive BAND_CHANGE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_CHANNEL_SPACE_ACTION)) {
                Utils.debugFunc("enter onReceive GET_CHANNEL_SPACE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_CHANNEL_SPACE_ACTION)) {
                Utils.debugFunc("enter onReceive SET_CHANNEL_SPACE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RDS_AF_SWITCH_MODE_ACTION)) {
                Utils.debugFunc( "enter onReceive GET_RDS_AF_SWITCH_MODE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_VOLUME_ACTION)) {
                Utils.debugFunc("enter onReceive GET_VOLUME_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_MONO_STEREO_MODE_ACTION)) {
                Utils.debugFunc("enter onReceive GET_MONO_STEREO_MODE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_MUTE_MODE_ACTION)) {
                Utils.debugFunc("enter onReceive GET_MUTE_MODE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_BAND_ACTION)) {
                Utils.debugFunc("enter onReceive GET_BAND_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_FREQUENCY_ACTION)) {
                Utils.debugFunc("enter onReceive GET_FREQUENCY_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RF_MUTE_MODE_ACTION)) {
                Utils.debugFunc("enter onReceive GET_RF_MUTE_MODE_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RSSI_THRESHHOLD_ACTION)) {
                Utils.debugFunc("enter onReceive GET_RSSI_THRESHHOLD_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_DEEMPHASIS_FILTER_ACTION)) {
                Utils.debugFunc("enter onReceive GET_DEEMPHASIS_FILTER_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RSSI_ACTION)) {
                Utils.debugFunc("enter onReceive GET_RSSI_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RDS_SYSTEM_ACTION)) {
                Utils.debugFunc("enter onReceive GET_RDS_SYSTEM_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.GET_RDS_GROUPMASK_ACTION)) {
                Utils.debugFunc("enter onReceive GET_RDS_GROUPMASK_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.ENABLE_RDS_ACTION)) {
                Utils.debugFunc("enter onReceive ENABLE_RDS_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.DISABLE_RDS_ACTION)) {
                Utils.debugFunc( "enter onReceive DISABLE_RDS_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_RDS_AF_ACTION)) {
                Utils.debugFunc( "enter onReceive SET_RDS_AF_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_RDS_SYSTEM_ACTION)) {
                Utils.debugFunc( "enter onReceive SET_RDS_SYSTEM_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_DEEMP_FILTER_ACTION)) {
                Utils.debugFunc( "enter onReceive SET_DEEMP_FILTER_ACTION "+ fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.PS_CHANGED_ACTION)) {
                Utils.debugFunc("enter onReceive PS_CHANGED_ACTION " + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_RSSI_THRESHHOLD_ACTION)) {
                Utils.debugFunc("enter onReceive SET_RSSI_THRESHHOLD_ACTION "+ fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.SET_RF_DEPENDENT_MUTE_ACTION)) {
                Utils.debugFunc("enter onReceive SET_RF_DEPENDENT_MUTE_ACTION "
                                        + fmAction,Log.INFO,printDebugInfo);
            ...
            if (fmAction.equals(FmReceiverIntent.COMPLETE_SCAN_DONE_ACTION)) {
                Utils.debugFunc("enter onReceive COMPLETE_SCAN_DONE_ACTION "+ fmAction,Log.INFO,printDebugInfo);
                ...
                int status = extras.getInt(FmReceiverIntent.STATUS, 0);
                Utils.debugFunc("noOfChannels" + noOfChannels,Log.INFO,printDebugInfo);

                for (int i = 0; i < noOfChannels; i++)

                {
                    Utils.debugFunc("channelList" + channelList[i],Log.INFO,printDebugInfo);
                }
            ...
            if (fmAction.equals(FmReceiverIntent.COMPLETE_SCAN_STOP_ACTION)) {
                Utils.debugFunc( "enter onReceive COMPLETE_SCAN_STOP_ACTION "+ fmAction,Log.INFO,printDebugInfo);
                Bundle extras = intent.getExtras();
                int status = extras.getInt(FmReceiverIntent.STATUS, 0);
                int channelValue = extras.getInt(
                        FmReceiverIntent.LAST_SCAN_CHANNEL, 0);
                Utils.debugFunc( "Last Scanned Channel Frequency before calling Stop Scan" + channelValue,Log.INFO,printDebugInfo);
                mHandler.sendMessage(mHandler.obtainMessage(
                        EVENT_COMPLETE_SCAN_STOP, status, channelValue));
            }

            if (fmAction.equals(FmReceiverIntent.MASTER_VOLUME_CHANGED_ACTION)) {
                Utils.debugFunc( "enter onReceive MASTER_VOLUME_CHANGED_ACTION "+ fmAction,Log.INFO,printDebugInfo);
    ...
    void getNewGain(int volume) {
        Utils.debugFunc( "getNewGain" + volume,Log.INFO,printDebugInfo);
        if (volume <= GAIN_STEP) {
            mVolume = MIN_VOLUME;
    ...
    public class NotificationsReceiver extends BroadcastReceiver {
        ...
        @Override
        public void onReceive(Context context, Intent intent) {
            Utils.debugFunc("Received Notification!",Log.VERBOSE,printDebugInfo);
            if (intent.hasExtra(EXTRA_COMMAND)) {
                Utils.debugFunc("Command: " + intent.getStringExtra(EXTRA_COMMAND),Log.VERBOSE,printDebugInfo);
    ...
    protected void onNewIntent(Intent intent) {
        Utils.debugFunc("called onNewIntent",Log.VERBOSE,printDebugInfo);
